...
- **UX** (`app/chat.tsx`): teclado tratado por **listeners manuais** (`Keyboard.addListener` `keyboardWill/DidShow`/`Hide` → altura real + `marginBottom`), não `KeyboardAvoidingView` (pouco confiável com `edgeToEdgeEnabled` + `softwareKeyboardLayoutMode:"resize"`); a lista usa `flex:1`. Respostas da IA renderizam `**negrito**` via `renderMarkdownBold` (parser leve, sem dependência nova — só nas bolhas da IA). Abaixo de cada resposta, legenda discreta do consumo do dia ("💬 X/limite mensagens · 🔎 Y/5 buscas"): `sendCfoMessage` **retorna** `msgCount`/`searchCount` (o valor recém-gravado — a UI não relê o AsyncStorage, evitando leitura-após-escrita stale que travava o contador em 0); a parte 🔎 é omitida no Free (sem busca). Entradas de **Mentor** e **Analisador** ficam só na aba IA — **removidas do Perfil** (rotas `/mentor` e `/analisador-precos` intactas).
