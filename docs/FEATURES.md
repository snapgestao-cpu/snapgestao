...
- **UX** (`app/chat.tsx`): teclado tratado por **listeners manuais** (`Keyboard.addListener` `keyboardWill/DidShow`/`Hide` → altura real + `marginBottom`), não `KeyboardAvoidingView` (pouco confiável com `edgeToEdgeEnabled` + `softwareKeyboardLayoutMode:"resize"`); a lista usa `flex:1`. Respostas da IA renderizam `**negrito**` via `renderMarkdownBold` (parser leve, sem dependência nova — só nas bolhas da IA). Consumo do dia em faixa persistente sob o header + legenda por resposta ("💬 X/limite mensagens · 🔎 Y/5 buscas"): o estado `dayUsage` é **hidratado 1x no mount** (`getCfoChatCount`/`getCfoSearchCount` — persiste ao sair e voltar no mesmo dia, não nasce em 0) e depois atualizado pelos valores que `sendCfoMessage` **retorna** (`msgCount`/`searchCount`, valor recém-gravado — sem reler o AsyncStorage a cada envio, evitando leitura-após-escrita stale); a parte 🔎 é omitida no Free (sem busca). Entradas de **Mentor** e **Analisador** ficam só na aba IA — **removidas do Perfil** (rotas `/mentor` e `/analisador-precos` intactas).
