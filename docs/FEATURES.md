...
- **`getOrGenerateWeeklyInsight(userId, plan, cycleStart)`**: semana = **segunda a domingo do calendário** (`mondayOfWeek`, independente do `cycle_start`). Agrega a semana (total gasto, top 3 potes, top 3 estabelecimentos, comparação % com a semana anterior — mesmo padrão do Mentor), gera via `callAI` (valores reais em R$ + trava anti-alucinação) e faz **upsert** em `weekly_insights` (`onConflict: user_id,week_start`) com `generated_at`.
- **Cache com invalidação por novidade** (`week_start` = segunda desta semana): sem cache → gera pela 1ª vez; cache **gerado hoje** → devolve direto (sem query extra, sem IA); cache gerado **num dia anterior** da mesma semana → só **regenera** se houver **≥1 transação nova** (`type=expense`, dentro da semana, `created_at > generated_at`, checado com `count/head`), reagregando a semana inteira e dando upsert na mesma linha; **sem novidade → devolve o cache, zero IA**. Segunda seguinte = nova `week_start` → volta ao caso "1ª geração". *Risco aceito*: editar/excluir lançamento antigo (sem criar novo) pode não regenerar (depende de `created_at` cobrir só criação) — documentado, mesmo padrão de risco aceito de outras partes.
- **Custo**: no máximo 1 chamada de IA por dia por usuário **e só quando há gasto novo**; refocus/reaberturas sem novidade reaproveitam o cache. **Não** consome a cota de `ai_tokens` (chama `callAI` direto, como a Camada 1).
...
- **UX** (`app/chat.tsx`): teclado tratado por **listeners manuais** (`Keyboard.addListener` `keyboardWill/DidShow`/`Hide` → altura real + `marginBottom`), não `KeyboardAvoidingView` (pouco confiável com `edgeToEdgeEnabled` + `softwareKeyboardLayoutMode:"resize"`); a lista usa `flex:1`. Respostas da IA renderizam `**negrito**` via `renderMarkdownBold` (parser leve, sem dependência nova — só nas bolhas da IA). Consumo do dia numa **única faixa fixa sob o header** ("💬 X/limite mensagens · 🔎 Y/5 buscas") — sem legenda por bolha. `dayUsage` é **hidratado 1x no mount** e atualizado pelos `msgCount`/`searchCount` que `sendCfoMessage` **retorna**; 🔎 omitido no Free. Os contadores usam **cache em memória** como fonte da verdade na sessão (com AsyncStorage como persistência best-effort entre sessões): `getCfo*Count = max(memória, persistido)`, o incremento parte desse max (progride 1→2→3… mesmo se o AsyncStorage não fizer round-trip) e `addCfoSearchCount` retorna o total acumulado (nunca zera numa mensagem sem busca). Entradas de **Mentor** e **Analisador** ficam só na aba IA — **removidas do Perfil** (rotas `/mentor` e `/analisador-precos` intactas).
