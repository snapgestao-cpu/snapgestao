...
**Versão atual**: `1.1.3` (versionCode 35) — release `v1.1.3-build35` (prerelease). Fluxo de release: dev/commits em `master` → `build.ps1` faz `git push origin master:main` + gera o APK; tag `vX.Y.Z-buildNN` em `main` + prerelease no GitHub. A branch `production` está obsoleta e **não** é a origem das releases.
...
- **Sugestão de pote (`smart_merchants`)**: a tabela é lida por `lib/smart-merchants.ts` — `suggestPotForMerchant(userId, name)` (1 lookup, usado nos modais) e `getMerchantPotMap(userId)` (batch, usado no import). Pré-seleciona o pote pelo estabelecimento em `NewExpenseModal`/`EditTransactionModal` (debounce 500ms) e pré-preenche por linha no `assign` do import. **Nunca** sobrescrever escolha manual do usuário (guard `potTouchedRef`); no Edit, só sugere quando o merchant é TROCADO (não no valor original). A coleta (`upsert`) roda ao salvar em ambos os modais.
